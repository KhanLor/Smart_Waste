...
if (in_array($date_range, ['7','30','90','365'], true)) {
    $where_conditions[] = "wr.created_at >= DATE_SUB(NOW(), INTERVAL ? DAY)";
...
// Get unread notifications for each report (for authority)
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <div>
                            <div class="d-flex align-items-center gap-2 mb-1">
                                <h2 class="mb-0">Waste Reports Management</h2>
                                <?php if ($unread_report_notifications > 0): ?>
                                    <span class="badge bg-danger rounded-pill" style="font-size: 0.75rem; padding: 0.35em 0.65em;">
                                        <?php echo $unread_report_notifications; ?> new report<?php echo $unread_report_notifications > 1 ? 's' : ''; ?>
                                    </span>
                                <?php endif; ?>
                            </div>
                                    ...
                                    <div class="card report-card <?php echo $report['priority']; ?> position-relative">
                                        <?php if (isset($report_notifications[$report['id']]) && $report_notifications[$report['id']] > 0): ?>
                                            <div class="position-absolute" style="top: 10px; right: 50px; z-index: 10;">
                                                <span class="badge bg-success rounded-pill" style="font-size: 0.7rem; padding: 0.3em 0.6em;">
                                                    <i class="fas fa-bell" style="font-size: 0.7rem;"></i> New
                                                </span>
                                            </div>
                                        <?php endif; ?>
                                        <div class="card-body">
                    ...
                    <?php if ($total_pages > 1): ?>
                        <nav aria-label="Reports pagination" class="mt-4">
                            <ul class="pagination justify-content-center">
                                <?php if ($page > 1): ?>
                                    <li class="page-item">
                                        <a class="page-link" href="?page=<?php echo $page - 1; ?>&search=<?php echo urlencode($search); ?>&status=<?php echo urlencode($status_filter); ?>&priority=<?php echo urlencode($priority_filter); ?>&type=<?php echo urlencode($type_filter); ?>&date_range=<?php echo urlencode($date_range); ?>">Previous</a>
                                    </li>
                                <?php endif; ?>
                                
                                <?php for ($i = max(1, $page - 2); $i <= min($total_pages, $page + 2); $i++): ?>
                                    <li class="page-item <?php echo $i === $page ? 'active' : ''; ?>">
                                        <a class="page-link" href="?page=<?php echo $i; ?>&search=<?php echo urlencode($search); ?>&status=<?php echo urlencode($status_filter); ?>&priority=<?php echo urlencode($priority_filter); ?>&type=<?php echo urlencode($type_filter); ?>&date_range=<?php echo urlencode($date_range); ?>"><?php echo $i; ?></a>
                                    </li>
                                <?php endfor; ?>
                                
                                <?php if ($page < $total_pages): ?>
                                    <li class="page-item">
                                        <a class="page-link" href="?page=<?php echo $page + 1; ?>&search=<?php echo urlencode($search); ?>&status=<?php echo urlencode($status_filter); ?>&priority=<?php echo urlencode($priority_filter); ?>&type=<?php echo urlencode($type_filter); ?>&date_range=<?php echo urlencode($date_range); ?>">Next</a>
                                    </li>
                                <?php endif; ?>
                            </ul>
                        </nav>
    ...
    <script>
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        ...
                        document.getElementById('reportModalBody').innerHTML = `
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <span class="text-muted">${report.resident.name}</span>
                                        </div>
                                        <div class="col-md-6">
                                            <strong>Email:</strong><br>
                                            <span class="text-muted">${report.resident.email}</span>
                                        </div>
                                    </div>
                                    <div class="row mt-2">
                                        <div class="col-md-6">
                                            <strong>Phone:</strong><br>
                                            <span class="text-muted">${report.resident.phone || 'Not provided'}</span>
                                        </div>
                                        <div class="col-md-6">
                                            <strong>Address:</strong><br>
                                            <span class="text-muted">${report.resident.address || 'Not provided'}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            ${report.assigned_to ? `
                            <h6 class="mb-3">Assigned Collector</h6>
                            <div class="card mb-3">
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <strong>Name:</strong><br>
                                            <span class="text-muted">${report.assigned_to.name}</span>
                                        </div>
                                        <div class="col-md-6">
                                            <strong>Email:</strong><br>
                                            <span class="text-muted">${report.assigned_to.email}</span>
                                        </div>
                                    </div>
                                    <div class="row mt-2">
                                        <div class="col-md-6">
                                            <strong>Phone:</strong><br>
                                            <span class="text-muted">${report.assigned_to.phone || 'Not provided'}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        function updateBadgeCounts() {
            ...
            if (headerBadge) {
                ...
                if (newCount > 0) {
                    headerBadge.textContent = newCount + ' new report' + (newCount > 1 ? 's' : '');
                } else {
                    headerBadge.remove();
                }
            }
            
            // Update the total reports count in header
            const totalReportsElement = document.querySelector('.h4.text-danger');
            if (totalReportsElement) {
                // Optionally refresh this if needed
            }
        }
    </script>
</body>
